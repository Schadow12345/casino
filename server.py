...
def init_db():
    with LOCK, db() as c:
        ...
        if "luck" not in have:
            c.execute("ALTER TABLE users ADD COLUMN luck INTEGER NOT NULL DEFAULT 50")
...
def get_luck(c, uid):
    r = c.execute("SELECT luck FROM users WHERE id=?", (uid,)).fetchone()
    return int(r["luck"]) if r and r["luck"] is not None else 50


def luck_desire(luck):
    """1 = Gewinn erzwingen, -1 = Niederlage erzwingen, 0 = fair."""
    if luck >= 100:
        return 1
    if luck <= 1:
        return -1
...
SLOT_PAY3 = [11, 17, 25, 50, 110, 280, 900]
SLOT_PAY2_CHERRY = 1
SLOT_BETS = [5, 10, 25, 50, 100, 250]
JACKPOT_SEED = 2000
JACKPOT_FEED_PCT = 2
JACKPOT_CHANCE = 1 / 900
WHEEL_PRIZES = [50, 100, 100, 150, 200, 250, 400, 1000]
...
def slots_spin(c, uid, data):
    ...
    wins, total = eval_grid(grid)
    if desire != 0:
        best = (grid, wins, total)
        for _ in range(800):
            g = [[rng.choices(range(7), SLOT_WEIGHTS)[0] for _ in range(3)] for _ in range(3)]
            w, t = eval_grid(g)
            if (desire == 1 and t > best[2]) or (desire == -1 and t < best[2]):
                best = (g, w, t)
            if desire == 1 and best[2] > bet:
                break
            if desire == -1 and best[2] < bet:
                break
        grid, wins, total = best
...
def roulette_spin(c, uid, data):
    ...
    def evaluate(n):
        ...
        return color, payout, results

    n = rng.randint(0, 36)
    color, payout, results = evaluate(n)
    if desire != 0:
        best = (n, color, payout, results)
        for _ in range(800):
            nn = rng.randint(0, 36)
            cc, pp, rr = evaluate(nn)
            if (desire == 1 and pp > best[2]) or (desire == -1 and pp < best[2]):
                best = (nn, cc, pp, rr)
            if desire == 1 and best[2] > total:
                break
            if desire == -1 and best[2] < total:
                break
        n, color, payout, results = best
...
def bj_card_score(hand, card):
    total, _ = bj_value(hand + [card])
    if total > 21:
        return -1000.0
    if len(hand) == 1 and total == 21:
        return 500.0
    return float(total)


def bj_draw(deck, hand, bias, favor):
    """Zieht eine Karte. bias -1..1, favor +1 = eigene Hand, -1 = Gegner."""
    if not deck:
        raise ApiError("Der Kartenstapel ist leer.")
    strength = abs(bias)
    if strength > 0 and random.random() < strength:
        want_strong = (bias > 0) == (favor > 0)
        key = lambda card: bj_card_score(hand, card)
        pick = max(deck, key=key) if want_strong else min(deck, key=key)
        deck.remove(pick)
        return pick
    return deck.pop()
...
def bj_finish(c, uid, st):
    ...
    if need_dealer:
        bias = st.get("bias", 0.0)
        while bj_value(st["dealer"])[0] < 17:
            st["dealer"].append(bj_draw(deck, st["dealer"], bias, -1))
    for bot in st["bots"]:
...
def bj_start(c, uid, data):
    ...
    player = []
    for _ in range(2):
        player.append(bj_draw(deck, player, bias, +1))
    bots = [{"name": "Max", "hand": [deck.pop(), deck.pop()]},
            {"name": "Luna", "hand": [deck.pop(), deck.pop()]}]
    dealer = []
    for _ in range(2):
        dealer.append(bj_draw(deck, dealer, bias, -1))
    st = {
        "deck": deck, "bet": bet, "phase": "player", "doubled": False,
        "bias": bias, "player": player, "bots": bots, "dealer": dealer,
    }
    save_state(c, uid, "blackjack", st)
...
def bj_action(c, uid, data):
    ...
    act = data.get("action")
    if act == "hit":
        st["player"].append(bj_draw(st["deck"], st["player"], st.get("bias", 0.0), +1))
...
def pk_hole_score(cards):
    a, b = sorted((rv(cards[0]), rv(cards[1])), reverse=True)
    s = a * 2 + b
    if a == b:
        s += 40
...
def pk_draw(deck, hand, bias, favor):
    strength = abs(bias)
    if strength > 0 and random.random() < strength:
        want_strong = (bias > 0) == (favor > 0)
        best = None
        for card in deck:
            s = pk_hole_score(hand + [card]) if hand else rv(card[0])
            if best is None or (want_strong and s > best[0]) or (not want_strong and s < best[0]):
                best = (s, card)
        pick = best[1]
        deck.remove(pick)
        return pick
...
def p_showdown(c, uid, st):
    ...
    if len(alive) == 1:
        ...
        reveal = True
        desire = luck_desire(get_luck(c, uid))
        if desire == 1 and 0 in alive:
            winners = [0]
            reveal = False
        elif desire == -1 and 0 in alive and len(alive) > 1:
            others = [i for i in alive if i != 0]
            top_other = max(scores[i] for i in others)
            winners = [i for i in others if scores[i] == top_other]
    pot = st["pot"]
...
def poker_start(c, uid, data):
    ...
    for fav in (+1, -1, -1):
        h = []
        for _ in range(2):
            h.append(pk_draw(deck, h, bias, fav))
        hands.append(h)
    st = {
...
def dice_roll(c, uid, data):
    ...
    payout, results = evaluate(dice)
    if desire != 0:
        best = (dice, payout, results)
        for _ in range(800):
            dd = [rng.randint(1, 6) for _ in range(3)]
            pp, rr = evaluate(dd)
            if (desire == 1 and pp > best[1]) or (desire == -1 and pp < best[1]):
                best = (dd, pp, rr)
            if desire == 1 and best[1] > total:
                break
            if desire == -1 and best[1] < total:
                break
        dice, payout, results = best
    s3 = sum(dice)
...
def user_info(c, uid):
    u = c.execute("SELECT username,balance,last_bonus,last_wheel,daily_streak,win_streak,best_streak,luck FROM users WHERE id=?",
                  (uid,)).fetchone()
    now = time.time()
...
def testgive(c, uid, data):
    ...
    if not isinstance(amount, int) or isinstance(amount, bool) or abs(amount) > TEST_MAX:
        raise ApiError(f"Betrag muss zwischen -{TEST_MAX} und {TEST_MAX} liegen.")
    if amount > 0:
        adjust(c, uid, amount)
        ...
        if new != bal:
            c.execute("UPDATE users SET balance=? WHERE id=?", (new, uid))
            log_tx(c, uid, "Testmenü", new - bal, "Testmenü (abgezogen)")
    info = user_info(c, uid)
    info["given_amount"] = amount
    return info


def testset(c, uid, data):
    code = str(data.get("code", "")).strip()
    if not secrets.compare_digest(code.encode(), TEST_CODE.encode()):
        raise ApiError("Dieser Code ist nicht gültig.")
    sets, params = [], []
    if "luck" in data:
        ...
        if not isinstance(v, int) or isinstance(v, bool) or not (1 <= v <= 100):
            raise ApiError("Glück muss zwischen 1 und 100 liegen.")
        sets.append("luck=?"); params.append(v)
    if "daily_streak" in data:
        v = data["daily_streak"]
        if not isinstance(v, int) or isinstance(v, bool) or not (0 <= v <= 100000):
            raise ApiError("Tages-Serie ungültig.")
        sets += ["daily_streak=?", "best_daily=MAX(best_daily,?)"]; params += [v, v]
    if "win_streak" in data:
        v = data["win_streak"]
        if not isinstance(v, int) or isinstance(v, bool) or not (0 <= v <= 100000):
            raise ApiError("Siegesserie ungültig.")
        sets += ["win_streak=?", "best_streak=MAX(best_streak,?)"]; params += [v, v]
    if "streak_stake" in data:
        v = data["streak_stake"]
        if not isinstance(v, int) or isinstance(v, bool) or not (0 <= v <= 10**12):
            raise ApiError("Serien-Einsatz ungültig.")
        sets.append("streak_stake=?"); params.append(v)
    if sets:
        params.append(uid)
...
def main():
    init_db()
    # Alte Sessions verwerfen -> man muss sich nach jedem Neustart neu anmelden.
    with LOCK, db() as c:
