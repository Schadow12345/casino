...
def init_db():
    with LOCK, db() as c:
        ...
        for col in ("daily_streak", "best_daily", "win_streak", "best_streak", "streak_stake", "last_wheel", "luck"):
            if col not in have:
...
def slots_spin(c, uid, data):
    ...
    def gen_grid():
        return [[rng.choices(range(7), SLOT_WEIGHTS)[0] for _ in range(3)] for _ in range(3)]

    def grid_has_win(g):
        for line in SLOT_LINES:
            s = [g[r][col] for r, col in line]
            if s[0] == s[1] == s[2] or (s[0] == s[1] == 0):
                return True
        return False
    ...
    luck = get_luck(c, uid)
    if luck > 0:
        want_win = rng.random() < luck / 100
        for _ in range(40):
            if grid_has_win(grid) == want_win:
                break
            grid = gen_grid()
    line_bet = bet / 5
    wins, total = [], 0
...
def roulette_spin(c, uid, data):
    ...
    def net_for(num):
        col = "green" if num == 0 else ("red" if num in RED else "black")
        tot = 0
        for t, v, a in clean:
            mult = 0
            if t == "num" and v == num:
                ...
                if (t == "red" and col == "red") or (t == "black" and col == "black") \
                        or (t == "even" and num % 2 == 0) or (t == "odd" and num % 2 == 1) \
                        or (t == "low" and num <= 18) or (t == "high" and num >= 19):
                    mult = 2
                elif t == "dozen" and (num - 1) // 12 + 1 == v:
                    mult = 3
            tot += a * mult
        return tot - total
    ...
    n = rng.randint(0, 36)
    if luck > 0:
        want_profit = rng.random() < luck / 100
        for _ in range(60):
            if (net_for(n) > 0) == want_profit:
                break
            n = rng.randint(0, 36)
...
def bj_finish(c, uid, st):
    ...
    target = None
    if luck > 0:
        target = "win" if rng.random() < luck / 100 else "lose"
    player_total = bj_value(st["player"])[0]
    player_bust = player_total > 21

    if target and not player_bust:
        if target == "win":
            # Dealer zieht so lange weiter, bis er sich überkauft (begrenzt gegen Endlosschleife)
            extra = 0
            while bj_value(st["dealer"])[0] <= 21 and extra < 8 and deck:
                st["dealer"].append(deck.pop())
                extra += 1
        else:
            # Dealer bleibt stehen, sobald er (auch unter 17) mit dem Spieler gleichzieht oder ihn schlägt
            while deck:
                dtotal = bj_value(st["dealer"])[0]
                if dtotal > 21 or (dtotal >= player_total and dtotal <= 21) or dtotal >= 17:
                    break
                st["dealer"].append(deck.pop())
    else:
    ...
    if target == "win":
        res = "blackjack" if bj_is_blackjack(st["player"]) and len(st["player"]) == 2 else "win"
    elif target == "lose":
        res = "lose"
    bet = st["bet"]
...
def p_showdown(c, uid, st):
    ...
    luck = get_luck(c, uid)
    if luck > 0 and 0 in alive:
        target = "win" if random.SystemRandom().random() < luck / 100 else "lose"
        if target == "win":
            winners = [0]
        elif target == "lose" and len(alive) > 1:
            others = [w for w in alive if w != 0]
            if others:
                winners = others
...
def dice_roll(c, uid, data):
    ...
    def net_for(d):
        ...
        for t, v, a in clean:
            mult = 0
            if t == "big" and not trip and 11 <= tot3 <= 17:
                mult = 2
            elif t == "small" and not trip and 4 <= tot3 <= 10:
                mult = 2
            elif t == "number":
                hits = d.count(v)
                if hits:
                    mult = 1 + hits
            elif t == "triple" and d[0] == d[1] == d[2] == v:
                mult = 151
            elif t == "any_triple" and trip:
                mult = 31
            tot += a * mult
        return tot - total
...
def user_info(c, uid):
    ...
    return {"username": u["username"], "balance": u["balance"], "bonus_in": wait,
            ...
            "win_streak": u["win_streak"], "best_streak": u["best_streak"], "luck": u["luck"],
...
def testset(c, uid, data):
    ...
    if "luck" in data:
        ...
        if not isinstance(v, int) or isinstance(v, bool) or v < 0 or v > 100:
            raise ApiError("Glück muss zwischen 0 und 100 liegen.")
        fields["luck"] = v
    if "win_streak" in data:
        v = data["win_streak"]
        if not isinstance(v, int) or isinstance(v, bool) or v < 0 or v > 1_000_000:
            raise ApiError("Ungültiger Wert für die Siegesserie.")
        fields["win_streak"] = v
    if "daily_streak" in data:
        v = data["daily_streak"]
        if not isinstance(v, int) or isinstance(v, bool) or v < 0 or v > 100_000:
            raise ApiError("Ungültiger Wert für die Tagesserie.")
        fields["daily_streak"] = v
        fields["last_bonus"] = time.time() - BONUS_COOLDOWN   # naechster Bonus setzt die Serie fort
    if not fields:
        raise ApiError("Nichts zum Setzen ausgewählt.")
    best_streak_val = fields.get("win_streak", 0)
    best_daily_val = fields.get("daily_streak", 0)
    sets = ", ".join(f"{k}=?" for k in fields)
    c.execute(
        f"UPDATE users SET {sets}, best_streak=MAX(best_streak,?), best_daily=MAX(best_daily,?) WHERE id=?",
        (*fields.values(), best_streak_val, best_daily_val, uid),
    )
    info = user_info(c, uid)
    info["set_fields"] = list(fields.keys())
    return info
